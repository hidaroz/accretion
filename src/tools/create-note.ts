...
export function registerCreateNote(server: McpServer, registry: VaultRegistry): void {
  server.registerTool(
    "create_note",
    {
    ...
    async ({ vault: vaultId, path: notePath, content, title, tags, frontmatter: extraFm }) => {
      try {
        ...
        const note = await ctx.vault.create(notePath, content, fm);
